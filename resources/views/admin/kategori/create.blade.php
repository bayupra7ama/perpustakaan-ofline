...
@section('title', 'Tambah Kategori • Perpustakaan SMPN 8 Bengkalis')
            ...
            <form method="POST" action="{{ route('admin.kategori.store') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                              class="w-full px-3 py-2 text-xs rounded-lg border border-slate-200
                                     focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">{{ old('description') }}</textarea>
                    @error('description')
                    <p class="mt-1 text-[11px] text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
